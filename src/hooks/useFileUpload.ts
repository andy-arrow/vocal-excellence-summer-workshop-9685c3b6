...
export type FileType = 'audioFile1' | 'audioFile2' | 'cvFile' | 'recommendationFile';

export const useFileUpload = (fileType: FileType | string) => {
  ...
  useEffect(() => {
    if (['audioFile1', 'audioFile2', 'cvFile', 'recommendationFile'].includes(fileType)) {
      const existingFile = applicationFilesStore.getFile(fileType as FileType);
  ...
  const handleFileUpload = async (file: File) => {
    try {
      ...
      setTimeout(() => {
        ...
        if (['audioFile1', 'audioFile2', 'cvFile', 'recommendationFile'].includes(fileType)) {
          applicationFilesStore.setFile(fileType as FileType, file);
        }
  ...
  const reset = () => {
    ...
    if (['audioFile1', 'audioFile2', 'cvFile', 'recommendationFile'].includes(fileType)) {
      applicationFilesStore.setFile(fileType as FileType, null);
    }
